...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        if len(errors_lst) == 0:                                                              
            registration_complete_html = ["Congratulations! You have registered successfully. Now let's get your favourite drone from our products page!!!Happy Droning", "Go Back Home"]   
            ...
            for row in rows:
                if user_to_create["Username"] == str(row[1]):
                    num_key = row[0]
                print(user_to_create["Username"], row[1], type(user_to_create["Username"]), type(row[1]), row[0])
            print(num_key, registration_complete_html)
